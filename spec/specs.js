describe('equilateral', function() {
  it("is true for a triangle with three equal sides", function() {
    expect(equilateral(3, 3, 3)).to.equal(true);
  });
});

describe('isosceles', function() {
  it("is true for a triangle with two equal sides", function() {
    expect(isosceles(3, 3, 2)).to.equal(true);
  });
});
    
describe('scalene', function() {
  it("is true for a triangle with no equal sides", function() {
    expect(scalene(3, 4, 5)).to.equal(true);
  });
});






/*describe('leapYear', function() {
  it("is true for a year that is divisible by 400", function() {
    expect(leapYear(1600)).to.equal(true);
  });
    
  it("is false for a year that is not divisible by 4", function() {
    expect(leapYear(1999)).to.equal(false);
  });
  
  it("is true for most years divisible by 4", function() {
    expect(leapYear(2012)).to.equal(true);
  });
    
  it("is false for most years divisible by 100", function () {
    expect(leapYear(1900)).to.equal(false);
  });
});*/